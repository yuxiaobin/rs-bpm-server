...
public interface WfInstHistMapper extends CommonMapper<WfInstHist> {
	...
	@Select({
				"<script>"
				+ "select task.task_descp_disp,a.ref_mkid,a.inst_num,a.curr_assigners, a.rs_wf_id "+
					"from wf_inst_hist hist "+
					"join ("+
					"select max(opt_seq) as opt_seq, inst.inst_id, ref_mkid,inst.inst_num,inst.curr_assigners, inst.task_id_curr, inst.RS_WF_ID "+
					...
					"</if>"+
					"group by inst.inst_id,ref_mkid,inst_num,curr_assigners "+
	...
	@Results({
        ...
        @Result(property="rsWfId",column="rs_wf_id",javaType=String.class),
